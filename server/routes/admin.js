...
router.get("/users", verifyFirebaseToken, checkRole("admin"), async (req, res) => {
  try {
    ...
    res.status(200).json({
      users,
      lastId: users.length > 0 ? users[users.length - 1].uid : null,
...
router.patch("/users/:id/role", verifyFirebaseToken, checkRole("admin"), async (req, res) => {
  try {
